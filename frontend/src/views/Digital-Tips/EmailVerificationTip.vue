<template>
  <div class="tip-root">
    <section class="intro">
      <h3>Practice: Find the verification email</h3>
      <p>Verification messages don’t always hit the Inbox. Use folders or search to find the latest code, then verify.</p>
    </section>

    <section class="controls">
      <input v-model="query" class="input" type="text" placeholder="Search mail (try: verify, code, YourShop)" />
...
// Weighted folder distribution to feel realistic
...
const latestCode = ref(generateCode()) // active/valid code only
const entered = ref('')
const ok = ref(false)
...
function generateCode() {
  // cryptographically strong 6-digit string (leading zeros allowed)
...
function seedMailbox() {
  ...
  injectVerificationEmail('Verify your email to finish signup', latestCode.value, /* newEmail= */ true)
}

function injectVerificationEmail(subject, code, newEmail = false) {
  // expire any prior verification emails when new one arrives
  if (newEmail) {
...
function resend() {
  ...
  injectVerificationEmail('Verify your email (new code)', latestCode.value, /* newEmail= */ true)
}
...
const displayTabs = computed(() => {
  return showAllMail.value ? BASE_TABS : BASE_TABS.filter(t => t.key !== 'All')
})
...
.btn-link { background:transparent; border:0; text-decoration:underline; cursor:pointer; }
